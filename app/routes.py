...
def api_metadata_by_study(study_id):
    ...
    return generate_metadata_dataset (study_id, study_group_id, sample_ids, sample_delim)
...
# optional parameters can be provided as get or post parameters
@app.route('/api/sampleinfo/studygroup_datasettype/<study_group_ids>/<dataset_type_id>', methods=('get', 'post'))
def api_sampleinfo_by_studygroup_datasettype(study_group_ids, dataset_type_id):
    # get request parameters from get or post, from query string or form
    ...
    return generate_sampleinfo_dataset(study_group_ids, dataset_type_id, aliquot_ids, aliquot_delim, aliquot_id_contains)
...
def api_sampleinfo_dataset():
    ...
    aliquot_delim = request.values.get('aliquot_delim')
    aliquot_id_contains = request.values.get('aliquot_id_contains')
    dataset_type_id = request.values.get('aliquot_delim')
    study_group_ids = request.values.get('study_group_ids')
    return generate_sampleinfo_dataset(study_group_ids, dataset_type_id, aliquot_ids, aliquot_delim, aliquot_id_contains)
    # return 'api_sampleinfo_dataset, study_group_id = {}, sample ids = {}, sample_delim = {}, study_id = {}'. \
    #     format(study_group_id, sample_ids, sample_delim, study_id)
...
def generate_metadata_dataset (study_id, study_group_id, sample_ids, sample_delim):
    mcfg = cm2.get_main_config()
    mlog, mlog_handler = cm2.get_logger(get_client_ip())
    process_name = inspect.stack()[1][3]
    ...
    result, columns, err = rp.get_dataset(mcfg, mlog, dataset_name,
                                          study_id = study_id, study_group_id = study_group_id,
...
def generate_sampleinfo_dataset (study_group_ids, dataset_type_id, aliquot_ids, aliquot_delim, aliquot_id_contains):
    ...
    # get the dataset from the database
    result, columns, err = rp.get_dataset(mcfg, mlog, dataset_name,
                                          study_group_ids = study_group_ids, dataset_type_id = dataset_type_id,
